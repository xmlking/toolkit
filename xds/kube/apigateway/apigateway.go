...
// xds.chinthagunta.io/api-gateway: Comma-separated list of API Gateway virtual servers. Only alphanumeric characters and dash allowed
// xds.chinthagunta.io/grpc-service: Comma-separated list of gRPC fully qualified service name (pkg.name.ServiceName)
	...
	for _, svc := range services {
		...
		for _, name := range apiGateways {
			if !nameRegex.MatchString(name) {
				log.Warn().Str("component", "xds").
					Msgf("Service %s/%s API Gateway %s does not match regex %s", svc.Namespace, svc.Name, name, nameRegex.String())
				continue outer
			}
		}
		...
		if !hasGrpcPort {
			log.Warn().Str("component", "xds").
				Msgf("Service %s/%s has API Gateway annotation but no grpc named port", svc.Namespace, svc.Name)
			continue
		}
